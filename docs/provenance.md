...
The map's focus state is display-only. Touching/holding a circle or starting a map drag on one selects it without changing saved positions or organization. The selected circle lifts 6%; viewport depth gently scales central circles forward (0.90–1.04×), and nearby circles move outward by at most seven layout points. Related circles/edges are highlighted using the existing shared-source/tag relationships; unrelated circles soften without disappearing. Connections interpolate with circle movement and end at their projected rims. A hold stays on the map; a quick tap opens the River directly with SwiftUI's [native zoom navigation transition](https://developer.apple.com/documentation/swiftui/zoomnavigationtransition), and Back returns to the same map. No preview drawer, decorative connections, media icons, numeric node badges, or idle animation is added. Focus can be cleared from the map background or its Clear focus control.

Adaptive materials become opaque with Reduce Transparency. Increased Contrast reduces fading. Following [Apple's motion guidance](https://developer.apple.com/design/human-interface-guidelines/motion), Reduce Motion keeps the selection/connection information but disables depth, focus scaling, neighbor displacement, and the custom zoom transition. Large accessibility text sizes retain the complete thread list. VoiceOver exposes selection and a Highlight connections action. A system selection haptic fires only when focus changes to a thread, not on every drag update. Haptic strength is controlled by iOS/device settings.
