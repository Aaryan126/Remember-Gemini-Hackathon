...
Every source memory is compared with at most eight locally retrieved candidate pages. Gemma may propose at most three concise, high-value changes. Candidate identifiers and cross-links must come from that bounded set. If Gemma finds no synthesis change but retrieval found a relevant match, Remember still preserves the source as evidence on that page without rewriting its text. The same evidence-only recovery contains malformed output after one bounded repair turn. Without a relevant candidate, Remember records a safe no-change result. Deterministic validation always controls what can be written.
