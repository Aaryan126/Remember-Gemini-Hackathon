...
- Map circles have subtle viewport depth and adaptive material surfaces. Touch/hold or start dragging a circle to lift it, highlight real connections, gently move nearby circles aside, and receive a selection haptic. A hold stays on the map; a quick tap enters the River with Apple’s native zoom transition. Clear focus or tap the map background to return to the overview. Reduce Motion disables depth, displacement, lift, and zoom transitions while retaining connection highlighting; Reduce Transparency uses opaque surfaces.
