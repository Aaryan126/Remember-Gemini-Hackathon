...
The v7 boundary asks for no more than three concise, distinct pages, explicitly distinguishes active projects from rules and requirements, and requires complete closing delimiters. The parser extracts balanced JSON objects while respecting quoted braces and escapes, so Markdown fences or surrounding prose cannot corrupt an otherwise valid object. If Gemma returns a valid no-change result while retrieval found a relevant page, a deterministic evidence-only recovery attaches the source to that one page while preserving its title and synthesis byte-for-byte. The same safe path contains malformed output after one bounded repair turn. Without a relevant candidate, Remember records an explicit, audited no-change result. Discarded no-change rows and failed rows from an older compiler version are requeued once automatically after an upgrade. Candidate allowlisting, the retrieval relevance boundary, field bounds, patch checks, and transactional persistence still control every mutation; raw malformed output is never logged or persisted.
